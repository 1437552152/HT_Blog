...
var item = localStorage.getItem("fingerPrint");
if (!item) {
    if (window.requestIdleCallback) {
        requestIdleCallback(function() {
            Fingerprint2.get(function(components) {
                ...
                console.log("设备指纹：" + murmur);
                localStorage.setItem("fingerPrint", murmur);
                $.ajaxSetup({
                    beforeSend: function(xhr) {
                        xhr.setRequestHeader("Finger-Print", murmur);
                    },
                });
            });
        });
    ...
    $.ajaxSetup({
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Finger-Print", item);
        },
    });
}
...
var cityLocation = sessionStorage.getItem("cityLocation");
if (!cityLocation) {
    $.ajax({
        url: "https://restapi.amap.com/v3/ip?key=2dab3e84e03832bcbc050bb9fc3abe91",
        dataType: "json",
        success: function(data) {
            if (data.status === "1") {
                var result = data.province + data.city;
                if (data.province === data.city) {
                    result = data.city;
                }
                console.log("根据IP查询地址位置结果：" + result);
                sessionStorage.setItem("cityLocation", result);
            } else {
                console.log("根据IP查询地址位置失败，原因：" + data.info);
            }
        },
    });
}
...
function getLocationInfoByIp() {
    return sessionStorage.getItem("cityLocation");
}